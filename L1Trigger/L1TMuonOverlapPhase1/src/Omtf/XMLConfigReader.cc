      ...
      for (unsigned int index = 1; index < 5; ++index) {
        ...
        if (aGPElement->getAttributeNode(xmliPt[index - 1])) {
          std::unique_ptr<GoldenPatternType> aGP =
              buildGP<GoldenPatternType>(aGPElement, aConfig, iItem, index, iGPNumber);
...
template std::vector<std::shared_ptr<GoldenPatternWithStat> > XMLConfigReader::readPatterns<GoldenPatternWithStat>(
    const L1TMuonOverlapParams &aConfig, const std::string &patternsFile);
