...
GpResultsToPt::GpResultsToPt(const std::vector<std::shared_ptr<GoldenPattern> >& gps,
  ...
  for (unsigned int iGP = 0; iGP < gps.size(); iGP++) {
    LogTrace("l1tMuBayesEventPrint") << " " << gps.at(iGP)->key() << " lower " << lowerGps[iGP]->key() << " higher "
...
unsigned int GpResultsToPt::lutAddres(AlgoMuons::value_type& algoMuon, unsigned int& candProcIndx) {
  ...
  LogTrace("l1tMuBayesEventPrint")
      << " iGP " << iGP << " candProcIndx " << candProcIndx << " iRefHit "
      << " " << iRefHit << " - lowerGps pdfSum " << lowerGps[iGP]->getResults()[candProcIndx][iRefHit].getPdfSum()
      << " " << lowerGps[iGP]->getResults()[candProcIndx][iRefHit].getFiredLayerCnt() << " - algoMuon pdfSum "
      << algoMuon->getGoldenPatern()->getResults()[candProcIndx][iRefHit].getPdfSum() << " "
      << algoMuon->getGoldenPatern()->getResults()[candProcIndx][iRefHit].getFiredLayerCnt() << " - higerGps pdfSum "
  ...
  LogTrace("l1tMuBayesEventPrint") << " addressL " << addressL << " addressR " << addressR << " address " << address
                                   << std::endl;
