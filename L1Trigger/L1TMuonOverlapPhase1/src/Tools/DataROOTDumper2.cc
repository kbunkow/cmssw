...
DataROOTDumper2::DataROOTDumper2(const edm::ParameterSet& edmCfg,
  ...
  edm::LogVerbatim("l1tMuBayesEventPrint")
      << " gps.size() " << gps.size() << " omtfConfig->nTestRefHits() " << omtfConfig->nTestRefHits()
      << " event.omtfGpResultsPdfSum.num_elements() " << event.omtfGpResultsPdfSum.num_elements() << endl;
...
void DataROOTDumper2::initializeTTree(std::string rootFileName) {
  ...
  if (dumpGpResults) {  //TODO not finished, probably has no sense
    ...
    edm::LogVerbatim("l1tMuBayesEventPrint") << " dumpGpResults "
...
void DataROOTDumper2::endJob() { edm::LogVerbatim("l1tMuBayesEventPrint") << " evntCnt " << evntCnt << endl; }
