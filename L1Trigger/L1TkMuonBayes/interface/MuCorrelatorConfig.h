...
public:
  ...
  virtual unsigned int nPhiBins() const { return phiBins; }
  ...
  virtual int etaToHwEta(double eta) const { return round(eta / etaUnit); }

  double ptUnit = 0.5;  // GeV/unit
  ///uGMT pt scale2 conversion
  virtual double hwPtToGev(int hwPt) const { return (hwPt - 1.) * ptUnit; }

  ///uGMT pt scale conversion: [0GeV, 0.5GeV) = 1 [0.5GeV, 1 Gev) = 2
  virtual int ptGevToHw(double ptGev) const { return (ptGev / ptUnit + 1); }
  ...
  virtual unsigned int nLayers() const { return layers; }
  ...
  unsigned int getBxToProcess() const { return bxToProcess; }
