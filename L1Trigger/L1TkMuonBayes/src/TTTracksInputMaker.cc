...
TrackingTriggerTracks TTTracksInputMaker::loadTTTracks(const edm::Event& event,
  ...
  if (ttTracksSource == SIM_TRACKS) {
      ...
      if (simTrackRef->eventId().bunchCrossing() == bx) {
        if ((abs(simTrackRef->type()) == 13 || abs(simTrackRef->type()) == 1000015) &&
          ...
          LogTrace("l1tMuBayesEventPrint")
              << __FUNCTION__ << ":" << __LINE__ << " bx " << bx << " adding ttTrack from simTrack: sim.type() "
              << simTrackRef->type() << " genpartIndex " << simTrackRef->genpartIndex() << " Beta() "
    ...
    for (unsigned int iTP = 0; iTP != trackingParticlesHandle->size(); iTP++) {
      ...
      if (trackingParticlePtr->eventId().bunchCrossing() ==
          bx) {  //to emulate the trigger rules we should process every track not only the muons!!!!
    ...
    for (unsigned int iTTTrack = 0; iTTTrack != tTTrackHandle->size(); iTTTrack++) {
      ...
      if (ttTrackPtr->getStubRefs().size() >=
          l1Tk_minNStub)  //TODO is this cut possible to apply in the firmware? there should be "Hit mask" so should be used whenever available
        addTTTrack(ttTracks, ttTrack, procConf);
...
void TTTracksInputMaker::addTTTrack(TrackingTriggerTracks& ttTracks,
  ...
  ttTrack->setPtBin(procConf->ptHwToPtBin(
      ttTrack
          ->getPtHw()));  //TODO when real hardware skale available, move to this function, and implement properly ptHwToPtBin
