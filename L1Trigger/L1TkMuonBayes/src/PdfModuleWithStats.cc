...
PdfModuleWithStats::PdfModuleWithStats(MuCorrelatorConfigPtr& config) : PdfModule(config), pdfHists(config->nLayers()) {
  ...
  edm::LogImportant("l1tMuBayesEventPrint")
      << __FUNCTION__ << ":" << __LINE__ << " PdfModuleWithStats: sigmaFactor: " << sigmaFactor << std::endl;
}
      ...
      for (unsigned int iRefLayer = 0; iRefLayer < pdfHists[iLayer][iEtaBin].size(); ++iRefLayer) {
        ...
        for (int ptBin = 0; ptBin < pdfHist->GetXaxis()->GetNbins(); ++ptBin) {
          ...
          const double pdfMaxLogVal =
              config
                  ->pdfMaxLogValue();  //the maximum value tha the logPdf can have (n.b. logPdf = pdfMaxLogVal - log(pdfVal) * pdfMaxLogVal / minPlog)
